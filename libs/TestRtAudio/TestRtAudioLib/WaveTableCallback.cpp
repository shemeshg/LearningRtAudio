...
RtWaveTableCallback::RtWaveTableCallback(int gWavetableLength) : gWavetableLength{gWavetableLength}
{
  gWavetable = (float *)std::calloc(gWavetableLength * 2, sizeof(float));
  assert(gWavetable);
  setupSine();

  RtGuiSlider rs("Frequency", gFrequency, 50, 5000, 1);
  RtGuiSlider rs2("Amplitude Db", gAmplitudeDb, -40, 0, 0.1);
                                ...
                                double &streamTime, RtAudioStreamStatus &status)
{
  ...
  int bufferPosition = 0;
  float gAmplitude = pow(10.0, gAmplitudeDb/20.0);
...
void RtWaveTableCallback::setupSine()
{
...
void RtWaveTableCallback::setupTriangle()
{
...
float RtWaveTableCallback::getLinearInterpolation(double gReadPointer, int chid)
{
